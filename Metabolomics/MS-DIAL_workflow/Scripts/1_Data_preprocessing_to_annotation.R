...
# Import GNPS data
gnps_pos <- read_csv('path_to/Raw_height_table.csv') #Output from MS_DIAL has the name as "Height_x_xxxxxxxxx.txt", conversion to .csv file is required
gnps_neg <- NULL
...
rowData(metab_pmp$imputed_results)$compound_name_gnps <- gnps_SE_names(gnps_df = gnps,
                                                                             metab_SE = metab_pmp$imputed_results)
...
# Load quality data
metab_quality <- read_csv('glog_curation.csv')
